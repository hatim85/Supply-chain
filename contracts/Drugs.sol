...
contract Drugs {
    ...
    struct Reputation {
        ...
        uint256 successfullTransactions;
    ...
    event BatchRevoked(uint256 batchId, address indexed owner);

    //Function for reputation
    function updateReputation(address participant, bool isSuccess) internal {
        Reputation storage rep = reputations[participant];
        rep.totalTransactions++;

        if (isSuccess) {
            rep.successfullTransactions++;
        } else {
            rep.failedTransactions++;
        }

        //percent of successfull transactions
        rep.reputationScore =
            (rep.successfullTransactions * 100) /
            rep.totalTransactions;
    }
    ...
    function getIoTDataHistory(
        string memory qrCode
    ) public view returns (IoTData[] memory) {
        require(drugIoTData[qrCode].length > 0, "No IoT data available");
        return drugIoTData[qrCode];
    }

    // Function to remove batch
    function revokeBatch(uint256 batchId) public {
        Batch storage batch = batches[batchId];
        require(
            batch.owner == msg.sender,
            "Only the owner can revoke the batch"
        );
        require(
            keccak256(bytes(batch.status)) != keccak256(bytes("Delivered")),
            "Cannot revoke a delivered batch"
        );
